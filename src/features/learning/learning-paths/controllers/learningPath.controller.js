...
export const createLearningPath = asyncHandler(async (req, res) => {
  const learningPath = await createLearningPathService(req.body, req.user.id);

  return res.status(201).json(
    new ApiResponse({
      message: Messages.LEARNING_PATH_CREATED,
      data: learningPathPresenter(learningPath),
    })
  );
});

export const getLearningPaths = asyncHandler(async (req, res) => {
  const result = await listLearningPathsService(req.query);
...
export const getLearningPathBySlug = asyncHandler(async (req, res) => {
  const learningPath = await getLearningPathBySlugService(req.params.slug);
...
export const updateLearningPath = asyncHandler(async (req, res) => {
  const learningPath = await updateLearningPathService(
    req.params.id,
    req.body,
    req.user.id
  );
...
export const updateLearningPathStatus = asyncHandler(async (req, res) => {
  const learningPath = await updateLearningPathStatusService(
    req.params.id,
    req.body.status,
    req.user.id
  );
